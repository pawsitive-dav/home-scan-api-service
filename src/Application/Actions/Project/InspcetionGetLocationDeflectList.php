...
class InspcetionGetLocationDeflectList extends MainAction
{
    protected function action(): Response
    {
        ...
        $sqlQuery = "SELECT
                        ...
                        isto.image_path,
                        isto.image_name
                    FROM inspection_location_deflect ild
                    LEFT JOIN member_info mi_created_by ON ild.created_by = mi_created_by.account_id
                    LEFT JOIN member_info mi_updated_by ON ild.updated_by = mi_updated_by.account_id
                    LEFT JOIN member_info mi_update_status_by ON ild.update_status_by = mi_update_status_by.account_id
                    LEFT JOIN inspection_storage isto ON ild.image_id = isto.image_id
                    WHERE 
                        ild.project_id = :project_id 
                        AND ild.inspection_id = :inspection_id 
                        AND ild.location_id = :location_id";
