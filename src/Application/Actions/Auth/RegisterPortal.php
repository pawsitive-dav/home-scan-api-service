...
class RegisterPortal extends AuthAction
{
    ...
    private function insertAccount($pdo, $UUID, $username, $passwordHashed, $dateNow)
    {
        ...
        $stmtAccount->bindValue(':username', $username);
        $stmtAccount->bindValue(':password', $passwordHashed);
        $stmtAccount->bindValue(':account_status', 'waiting');
        $stmtAccount->bindValue(':created_by', 'portal');
