<?php

declare(strict_types=1);

namespace App\Application\Actions\Project;

use Psr\Http\Message\ResponseInterface as Response;

class InspcetionEditLocationDeflectDetail extends MainAction
{
    protected function action(): Response
    {
        $input = $this->getFormData();
        $allKeys = ["project_id", "inspection_id", "location_id", "image_id", "deflect_detail"];
        $requiredKeys = ["project_id", "inspection_id", "location_id", "image_id"];

        if (!$this->validateInputBody($input, $allKeys, $requiredKeys)) {
            return $this->respondWithData("Bad Request", 400);
        }

        $pdo = $this->pdoConnect($_ENV['DB_PORTAL']);
        date_default_timezone_set("Asia/Bangkok");
        $date_now = date('Y-m-d H:i:s');
        $account_id = $this->request->getAttribute('tokenInfo')->data;

        $sqlQuery = "UPDATE inspection_location_deflect
                    SET
                        deflect_detail = :deflect_detail,
                        updated_at = :updated_at,
                        updated_by = :updated_by
                    WHERE
                        project_id = :project_id
                        AND inspection_id = :inspection_id
                        AND location_id = :location_id
                        AND image_id = :image_id";

        $stmt = $pdo->prepare($sqlQuery);
        $stmt->bindValue(':deflect_detail', $input['deflect_detail']);
        $stmt->bindValue(':updated_at', $date_now);
        $stmt->bindValue(':updated_by', $account_id);
        $stmt->bindValue(':project_id', $input['project_id']);
        $stmt->bindValue(':inspection_id', $input['inspection_id']);
        $stmt->bindValue(':location_id', $input['location_id']);
        $stmt->bindValue(':image_id', $input['image_id']);
        $result = $stmt->execute();

        if (empty($result)) return $this->respondWithData("Update Detail Fail", 404);

        return $this->respondWithData("Update Detail Successfuly");
    }
}
